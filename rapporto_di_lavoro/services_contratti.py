...
def contratti_td_in_scadenza(azienda, giorni: int = 30, oggi=None):
	"""
	Rapporti TD ancora attivi con data fine entro `giorni` da oggi (incluso oggi).
	Usato per alert rinnovo (preparazione entro 30 gg dalla scadenza).
	"""
	from .models import RapportoDiLavoro
	...
	return [r for r in qs if is_tempo_determinato(r.tipo_contratto)]
...
def contratti_td_scaduti_non_chiusi(azienda, oggi=None):
	"""TD con data fine passata ancora in stato attivo (da allineare con il comando di cessazione)."""
	from .models import RapportoDiLavoro
