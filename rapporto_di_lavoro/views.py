...
def lista_contratti_scadenza(request):
	"""Elenco contratti a termine in scadenza e scaduti non chiusi (dipendenti non cessati)."""
	from .services_contratti import contratti_td_in_scadenza, contratti_td_scaduti_non_chiusi
	...
	try:
		giorni_finestra = int((request.GET.get('giorni') or '120').strip())
	except (TypeError, ValueError):
		giorni_finestra = 120
	giorni_finestra = max(7, min(giorni_finestra, 730))

	prossimi_raw = contratti_td_in_scadenza(azienda_operativa, giorni=giorni_finestra, oggi=oggi)
