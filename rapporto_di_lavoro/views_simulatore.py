...
def _data_fine_effettiva_con_cessazione(dipendente, data_fine_contratto):
    """
    Allinea al profilo dipendente: ``data_cessazione`` in anagrafica chiude il rapporto
    prima della ``data_fine_rapporto`` del contratto se è precedente (stessa logica del
    riepilogo presenze / ``_periodo_rapporto_dipendente_per_mese``).
    """
    if dipendente is None:
        return data_fine_contratto
    dc = getattr(dipendente, 'data_cessazione', None)
    if dc is None:
        return data_fine_contratto
    if data_fine_contratto is None or dc < data_fine_contratto:
        return dc
    return data_fine_contratto
...
def _json_prefill_da_contratto_dipendente(request, dip_id: int, anno: int, mese: int) -> dict:
    ...
    df_pref = _data_fine_effettiva_con_cessazione(dip, rapporto.data_fine_rapporto)
    payload = {
        'ok': True,
        'fonte': 'contratto',
        ...
        'data_fine_rapporto': df_pref.isoformat() if df_pref else '',
...
def _json_prefill_da_proposta(request, proposta_id: int, anno: int, mese: int) -> dict:
    ...
    df_prop = prop.data_fine_rapporto
    if prop.dipendente_id:
        try:
            from anagrafiche.models import Dipendente

            _dip_p = Dipendente.objects.get(pk=prop.dipendente_id)
            df_prop = _data_fine_effettiva_con_cessazione(_dip_p, df_prop)
        except Dipendente.DoesNotExist:
            pass
    return {
        'ok': True,
        ...
        'data_fine_rapporto': df_prop.isoformat() if df_prop else '',
        'superminimo': str(sm),
        'divisore': div,
        'usa_dati_contratto': '',
...
def simulatore_paga(request):
    ...
    try:
        ...
        if usa_contr:
            ...
            data_fine = _data_fine_effettiva_con_cessazione(dip_ctx, rapporto.data_fine_rapporto)
        ...
        if dip_id_raw and not usa_contr:
            try:
                from anagrafiche.models import Dipendente
                _dip_cap = Dipendente.objects.get(pk=int(dip_id_raw))
                data_fine = _data_fine_effettiva_con_cessazione(_dip_cap, data_fine)
        ...
        if dip_id_raw and _tutti_zero:
            try:
                ...
                _azp = azienda or _dip_for_pres.azienda
                _di, _df = _periodo_rapporto_dipendente_per_mese(_dip_for_pres, _azp, anno, mese)
                _ps, _pe = _intervallo_mese_per_rapporto(anno, mese, _di, _df)
                if not (_ps and _pe):
                    raise ValueError('dipendente fuori periodo rapporto nel mese')
                _agg_pres = get_presenze_mese_aggregato(
                    _dip_for_pres, anno, mese, _azp, data_da=_ps, data_a=_pe,
                )
...
def api_presenze_simulatore(request):
    ...
    az_eff = azienda or dipendente.azienda
    ...
    if mese_raw:
        ...
        di, df = _periodo_rapporto_dipendente_per_mese(dipendente, az_eff, anno, mese)
        ps, pe = _intervallo_mese_per_rapporto(anno, mese, di, df)
        if ps and pe:
            result = get_presenze_mese_aggregato(
                dipendente, anno, mese, azienda, data_da=ps, data_a=pe,
            )
        else:
            ...
            result = get_presenze_mese_aggregato(
                ...
                mese,
                azienda,
                data_da=date(anno, mese, ult),
                data_a=date(anno, mese, 1),
            )
