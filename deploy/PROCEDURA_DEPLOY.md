...
5. **Deploy da sviluppo:** gli script in `deploy/` usano di default `GESPER_DEPLOY_HOST=ubuntu@gesper1.plazapretoria.it`. Su molte VPS (es. Aruba) è pratico usare **`root@gesper1.plazapretoria.it`** se `ubuntu` non ha chiave o è disabilitato (vedi **§0.1**). Configurare **accesso SSH a chiave** dal Mac (`ssh-copy-id`) così `sync-pwa-and-collectstatic.sh` non fallisce tra il primo `rsync` e il secondo `ssh` (due login separati).
