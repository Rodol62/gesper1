...
def riepilogo_ore_mese_sidebar(dipendente, azienda, anno: int, mese: int) -> dict:
    ...
    """
    from presenze.views import _get_config_orario_mese, _periodo_rapporto_dipendente_per_mese
    ...
    di_r, df_r = _periodo_rapporto_dipendente_per_mese(dipendente, azienda, anno, mese)
    ...
    if di_r:
        presenze_qs = presenze_qs.filter(data__gte=di_r)
    if df_r:
        presenze_qs = presenze_qs.filter(data__lte=df_r)
    presenze = list(presenze_qs)
...
def aggrega_presenze_per_motore(
    ...
    """
    from rapporto_di_lavoro.utils_calendario import get_festivita_mese
    from presenze.views import _periodo_rapporto_dipendente_per_mese
    ...
    di, df = _periodo_rapporto_dipendente_per_mese(dipendente, azienda, anno, mese)
    ...
    if di:
        presenze = presenze.filter(data__gte=di)
    if df:
        presenze = presenze.filter(data__lte=df)
