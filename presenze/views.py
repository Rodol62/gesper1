...
def _periodo_rapporto_dipendente_per_mese(dipendente, azienda, anno: int, mese: int):
    ...
    Se in anagrafica è valorizzata ``data_cessazione``, l'ultimo giorno di rapporto effettivo è
    il minimo tra quella data e la ``data_fine_rapporto`` del contratto (così dal giorno dopo
    la cessazione non si valorizzano presenze / ore nel riepilogo).
    """
    ...
    # Profilo dipendente: la cessazione in anagrafica chiude il periodo utile alle presenze
    # anche se il contratto in archivio ha ancora data_fine nulla (es. T.I.) o successiva.
    dc = getattr(dipendente, 'data_cessazione', None)
    if dc is not None:
        if data_fine is None or dc < data_fine:
            data_fine = dc
...
def _ultimo_mese_con_busta_attesa_scostamento(anno: int) -> int:
    """
    Ultimo mese (1..12) dell'anno per cui ha senso confrontare il cedolino con il calendario.

    Per l'anno solare **in corso** si considerano solo i mesi fino al **precedente** rispetto
    al mese calendario attuale: le buste del mese in esercizio (e successivi) non sono ancora
    disponibili dal consulente e genererebbero scostamenti fuorvianti.
    """
    oggi = date.today()
    if anno < oggi.year:
        return 12
...
def _build_rows_scostamento_cedolino(azienda, anno: int, mese: int, q_filter: str, ordina: str, filtro: str):
    ...
    from rapporto_di_lavoro.utils_presenze import (
        ...
        ore_cedolino_festivita_totali,
    )

    mesi = [mese] if mese else list(range(1, 13))
    limite_mese_cedolino = _ultimo_mese_con_busta_attesa_scostamento(anno)
    mesi = [m for m in mesi if m <= limite_mese_cedolino]
    oggi = date.today()
    if not mesi:
        stat = {
            'totale': 0,
            ...
            'senza_cedolino': 0,
            'visualizzate': 0,
            'limite_mese_cedolino': limite_mese_cedolino,
            'oggi_anno': oggi.year,
            'oggi_mese': oggi.month,
        }
        return [], stat, []

    dip_qs = dipendenti_per_riepilogo_mese(azienda, anno, mesi[0])
    if q_filter:
        qobj = (
            Q(cognome__icontains=q_filter)
            | Q(nome__icontains=q_filter)
            ...
            if ced:
                for v in VoceCedolinoMotoreV4.objects.filter(cedolino=ced):
                    ...
                    if code in cod_ord:
                        ...
                        dom_h += h
                fest_h = ore_cedolino_festivita_totali(dict(ced_by))
...
def _group_scostamento_per_dipendente(rows: list[dict], anno: int, ordina_gruppi: str) -> list[dict]:
    """
    Raggruppa le righe dipendente/mese in un blocco per dipendente (anno in pagina).

    ``righe_mese`` è ordinata dal **mese più recente** al meno recente.
    Totali: somma degli scostamenti mensili (cedolino − calendario) e somma dei valori assoluti.
    """
    by_id: dict[int, list[dict]] = defaultdict(list)
    for r in rows:
        by_id[r['dip'].id].append(r)
    q2 = Decimal('0.01')
    gruppi: list[dict] = []
    for did, lst in by_id.items():
        lst.sort(key=lambda x: -int(x['mese']))
        tot = sum((x['scostamento'] for x in lst), start=Decimal('0')).quantize(q2)
        tot_abs = sum((abs(x['scostamento']) for x in lst), start=Decimal('0')).quantize(q2)
        dip = lst[0]['dip']
        gruppi.append(
            {
                'dip': dip,
                'anno': anno,
                'righe_mese': lst,
                'totale_scostamento': tot,
                'totale_abs_scostamento': tot_abs,
                'nr_mesi_analizzati': len(lst),
                'nr_mesi_con_delta': sum(1 for x in lst if x['scostamento'] != 0),
                'nr_mesi_senza_cedolino': sum(1 for x in lst if not x['has_cedolino']),
                'id_collapse': f'scd-grp-{did}-{anno}',
            }
        )
    if ordina_gruppi == 'dipendente':
        gruppi.sort(key=lambda g: (g['dip'].cognome.lower(), g['dip'].nome.lower()))
    elif ordina_gruppi == 'scostamento':
        gruppi.sort(
            key=lambda g: (-abs(g['totale_scostamento']), g['dip'].cognome.lower(), g['dip'].nome.lower())
        )
    elif ordina_gruppi == 'mese_desc':
        gruppi.sort(
            key=lambda g: (
                -max((x['mese'] for x in g['righe_mese']), default=0),
                g['dip'].cognome.lower(),
                g['dip'].nome.lower(),
            )
        )
    elif ordina_gruppi == 'mese_asc':
        gruppi.sort(
            key=lambda g: (
                min((x['mese'] for x in g['righe_mese']), default=13),
                g['dip'].cognome.lower(),
                g['dip'].nome.lower(),
            )
        )
        ...
        gruppi.sort(
            key=lambda g: (-g['totale_abs_scostamento'], g['dip'].cognome.lower(), g['dip'].nome.lower())
        )
    return gruppi
...
def riepilogo_scostamento_cedolino(request):
    ...
    gruppi_scostamento = _group_scostamento_per_dipendente(rows, anno, ordina)
    stat['n_gruppi_dipendenti'] = len(gruppi_scostamento)
    lim = int(stat.get('limite_mese_cedolino') or 0)
    lim_nome = MESI_ITA[lim] if 1 <= lim <= 12 else ''
    return render(request, 'presenze/riepilogo_scostamento_cedolino.html', {
        ...
        'rows': rows,
        'gruppi_scostamento': gruppi_scostamento,
        'q_filter': q_filter,
        'ordina': ordina,
        'filtro': filtro,
        'stat': stat,
        'limite_mese_cedolino_nome': lim_nome,
    })
...
def export_scostamento_cedolino_excel(request):
    ...
    if not mesi:
        ...
        ws.append([
            'Nessun dato esportabile: per l’anno selezionato non risultano mesi inclusi nel confronto '
            'cedolino (per l’anno in corso solo fino al mese precedente al mese calendario attuale).',
        ])
        out = BytesIO()
        wb.save(out)
        out.seek(0)
...
def export_scostamento_cedolino_pdf(request):
    ...
    if not rows:
        story.append(Paragraph(
            'Nessun dato esportabile: per l’anno selezionato non risultano mesi inclusi nel confronto cedolino '
            '(per l’anno in corso solo fino al mese precedente al mese calendario attuale).',
            styles['Normal'],
        ))
    else:
        for r in rows:
