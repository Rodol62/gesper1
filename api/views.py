...
def _api_login_use_sandbox(username_raw: str, resolved_username: str) -> bool:
    """Login API verso DB sandbox (username dimostrativo o e-mail ``{user}@invalid.local`` del seed)."""
    from sandbox_dimostrativo.login_routing import is_sandbox_demo_login_identifier

    return is_sandbox_demo_login_identifier(username_raw) or is_sandbox_demo_login_identifier(
        resolved_username
    )
...
def login_view(request):
    ...
    from sandbox_dimostrativo.login_routing import SESSION_API_2FA_SANDBOX
    from sandbox_dimostrativo.state import set_sandbox_routing

    username_raw = request.data.get("username", "").strip()
    ...
    routing_sandbox = False
    try:
        use_sb_raw = _api_login_use_sandbox(username_raw, username_raw)
        if use_sb_raw:
            set_sandbox_routing(True)
            routing_sandbox = True
        ...
        use_sb = _api_login_use_sandbox(username_raw, username)
        if use_sb and not routing_sandbox:
            set_sandbox_routing(True)
            routing_sandbox = True

        from sandbox_dimostrativo.login_routing import (
            is_sandbox_demo_login_identifier,
            resolve_sandbox_demo_username_for_auth,
        )

        if is_sandbox_demo_login_identifier(username):
            username = resolve_sandbox_demo_username_for_auth(username)
        ...
        if user.totp_enabled:
            if use_sb:
                request.session[SESSION_API_2FA_SANDBOX] = True
...
def logout_view(request):
    """
    Chiude la sessione Django (cookie sessionid) associata alla richiesta.
    Chiamare dall’app in uscita con credentials: include; i JWT vanno rimossi lato client.
    """
    auth_logout(request)
    return Response(status=status.HTTP_204_NO_CONTENT)
...
def otp_verify(request):
    ...
    routing_sandbox = bool(request.session.get(SESSION_API_2FA_SANDBOX))
    if routing_sandbox:
        set_sandbox_routing(True)
    try:
        try:
            ...
            return Response(
                {"detail": "Sessione scaduta, ricomincia il login."},
        ...
        _backend = getattr(user, "backend", settings.AUTHENTICATION_BACKENDS[0])
...
def _stato_response(p):
    # Allineato a timbratura geo: orari solo precompilati (note senza src=) non
    # contano come turni reali — altrimenti la PWA mostra «3 turni» e blocca l’ingresso.
    from accounts.views_candidato import _ha_timbrature_reali

    if p is not None and not _ha_timbrature_reali(p):
        return {
            'turni': [],
            'in_servizio': False,
            'puo_entrare': True,
...
def _api_lat_lon_acc_from_request(request):
    """Coordinate da JSON o form (PWA / client API)."""
    from accounts.views_candidato import _to_float_or_none

    data = getattr(request, 'data', None)
    if not isinstance(data, dict):
        data = request.POST
    lat = _to_float_or_none(data.get('lat'))
    lon = _to_float_or_none(data.get('lon'))
    acc = _to_float_or_none(data.get('acc') or data.get('accuracy'))
    return lat, lon, acc
...
def checkin_stato(request):
    from accounts.views_candidato import _get_geo_config_value
    ...
    try:
        p = Presenza.objects.get(dipendente=dip, data=oggi)
    except Presenza.DoesNotExist:
        p = None
    body = _stato_response(p)
    geo_on = bool(_get_geo_config_value('presenze_geo_enabled', 'PRESENZE_GEO_ENABLED', True))
    body['geo_timbratura'] = {
        'attiva': geo_on,
        'richiede_coordinate': bool(_get_geo_config_value('presenze_geo_require_gps', 'PRESENZE_GEO_REQUIRE_GPS', True)) if geo_on else False,
        'enforce_perimetro': bool(_get_geo_config_value('presenze_geo_enforce_geofence', 'PRESENZE_GEO_ENFORCE_GEOFENCE', True)) if geo_on else False,
    }
...
def checkin_view(request):
    ...
    from accounts.views_candidato import timbratura_geo_processo, _get_geo_config_value
    ...
    geo_on = bool(_get_geo_config_value('presenze_geo_enabled', 'PRESENZE_GEO_ENABLED', True))
    if geo_on:
        lat, lon, acc = _api_lat_lon_acc_from_request(request)
        payload, st = timbratura_geo_processo(request, dip, 'checkin', lat, lon, acc)
        if not payload.get('ok'):
            err = {'detail': payload.get('error', 'Timbratura non consentita.')}
            err.update({k: v for k, v in payload.items() if k != 'error'})
            return Response(err, status=st)
        p = Presenza.objects.get(dipendente=dip, data=timezone.localdate())
        out = {'ora': payload['ora'], **_stato_response(p)}
        for k, v in payload.items():
            if k not in ('ok', 'azione', 'ora'):
                out[k] = v
        return Response(out, status=status.HTTP_201_CREATED)

    ora_ora = timezone.localtime().time().replace(second=0, microsecond=0)
...
def checkout_view(request):
    ...
    if geo_on:
        lat, lon, acc = _api_lat_lon_acc_from_request(request)
        payload, st = timbratura_geo_processo(request, dip, 'checkout', lat, lon, acc)
        if not payload.get('ok'):
            err = {'detail': payload.get('error', 'Timbratura non consentita.')}
            err.update({k: v for k, v in payload.items() if k != 'error'})
            return Response(err, status=st)
