...
class CustomLoginForm(AuthenticationForm):
    ...
    def clean(self) -> dict:
        ...
        # Django ≥5.1: ``self.username_field`` è il ``Field`` del modello, non il nome (es. ``"username"``).
        # ``cleaned_data`` usa sempre la chiave del campo del form, cioè ``User.USERNAME_FIELD``.
        User = get_user_model()
        uname_key = User.USERNAME_FIELD
        uname = (self.cleaned_data.get(uname_key) or "").strip()
        if uname and is_sandbox_demo_login_identifier(uname):
            if not getattr(django_settings, "GESPER_SANDBOX_ENABLED", False):
                raise forms.ValidationError(
                    "Hai usato le credenziali dimostrative ma la sandbox non è attiva. In locale usa "
                    "``DJANGO_SETTINGS_MODULE=settings`` (non ``settings_production``): il repo include "
                    "``.env.development`` che abilita la demo se non hai già ``GESPER_SANDBOX_ENABLED`` nel "
                    "tuo ``.env``. Riavvia il server, poi: python manage.py gesper_sandbox_migrate && "
                    "python manage.py gesper_sandbox_seed.",
                    code="sandbox_disabled",
                )
            canon = resolve_sandbox_demo_username_for_auth(uname)
            self.cleaned_data[uname_key] = canon
            if not User.objects.filter(**{uname_key: canon}).exists():
                raise forms.ValidationError(
                    "Utente demo assente nel database dimostrativo. Con GESPER_SANDBOX_ENABLED=1 esegui: "
                    "python manage.py gesper_sandbox_migrate && python manage.py gesper_sandbox_seed",
...
class CustomLoginView(LoginView):
    ...
    def form_valid(self, form):
        user = form.get_user()
        from sandbox_dimostrativo.login_routing import is_sandbox_demo_username

        if is_sandbox_demo_username(user.get_username()):
            self.request.session["gesper_login_pending_sandbox"] = True
...
def login_email_otp(request):
    ...
    if request.method == 'POST':
        if request.POST.get('cancel'):
            ...
            request.session.pop('gesper_login_pending_sandbox', None)
...
def profile(request):
    ...
    context['gesper_sandbox_abilitato'] = getattr(django_settings, "GESPER_SANDBOX_ENABLED", False)
    context['gesper_sandbox_sessione_attiva'] = bool(request.session.get("gesper_sandbox_attiva"))
    _names = getattr(django_settings, "GESPER_SANDBOX_USERNAMES", frozenset())
    context["gesper_sandbox_username_demo"] = min(_names) if _names else "demo"
    context["gesper_demo_mask_contacts"] = bool(getattr(request, "gesper_sandbox", False))
    ...
    # Dati portale lavoratore: non per consulente (profilo dedicato sopra nel template)
    if user.has_ruolo('dipendente') and not user.has_ruolo('consulente'):
        try:
...
def dashboard_admin(request):
    """Unificato con ``/moduli/``: reindirizza al portale (stesso contenuto)."""
    return redirect('centro_moduli')
...
def edit_profile(request):
    ...
    return render(
        ...
        {
            "user": user,
            "gesper_demo_mask_contacts": bool(getattr(request, "gesper_sandbox", False)),
        },
    )
