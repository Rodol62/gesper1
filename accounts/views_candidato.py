...
def timbratura_geo_processo(request, dip, azione, lat, lon, acc):
    """
    Timbratura con geolocalizzazione e stessi controlli turni del portale.
    Ritorna (dict_json, status_http) per JsonResponse/DRF Response.

    Coordinate centro: sede lavorativa azienda se valorizzata, altrimenti
    default globale (Impostazioni di sistema). Geofence se
    presenze_geo_enforce_geofence è attivo.
    """
    if not _get_geo_config_value('presenze_geo_enabled', 'PRESENZE_GEO_ENABLED', True):
        ...
        return ({'ok': False, 'error': 'Timbratura geolocalizzata non abilitata.'}, 403)

    if not dip:
        return ({'ok': False, 'error': 'Dipendente non associato.'}, 400)
    ...
    if require_gps and not gps_ok and not test_exempt:
        ...
        return ({
            'ok': False,
            'error': (
                'Geolocalizzazione richiesta. Attiva il GPS e riprova. '
                'Avvertenza - questa rilevazione è stata fatta fuori dal perimetro della sede di lavoro.'
            ),
        }, 400)
    ...
    enforce_geofence = bool(
        _get_geo_config_value('presenze_geo_enforce_geofence', 'PRESENZE_GEO_ENFORCE_GEOFENCE', True)
    )
    ...
    if enforce_geofence and gps_ok and distanza is not None and distanza > radius_m and not test_exempt:
        ...
        return ({
            ...
            'error': (
                'Avvertenza - questa rilevazione è stata fatta fuori dal perimetro della sede di lavoro. '
                f'Fuori area timbratura ({int(distanza)}m > {int(radius_m)}m).'
            ),
            'distanza_m': distanza,
            'raggio_m': radius_m,
        }, 403)
    ...
    if p.causale != 'P':
        ...
        return ({'ok': False, 'error': f'Presenza non timbrabile: causale {causale_label}.'}, 400)
    ...
    if azione == 'checkin':
        if p.ora_entrata and not p.ora_uscita:
            ...
            return ({
                'ok': False,
                'error': f'Turno 1 già aperto dalle {ora_t1.strftime("%H:%M")}. Registra prima il check-out.',
                'stato': 'turno1_aperto',
            }, 400)
        if p.ora_entrata2 and not p.ora_uscita2:
            ...
            return ({
                'ok': False,
                'error': f'Turno 2 già aperto dalle {ora_t2.strftime("%H:%M")}. Registra prima il check-out.',
                'stato': 'turno2_aperto',
            }, 400)
        if p.ora_entrata3 and not p.ora_uscita3:
            ...
            return ({
                'ok': False,
                'error': f'Turno 3 già aperto dalle {ora_t3.strftime("%H:%M")}. Registra prima il check-out.',
                'stato': 'turno3_aperto',
            }, 400)
        ...
        if not p.ora_entrata:
            ...
            if is_test:
                ...
                return ({'ok': False, 'error': 'Limite raggiunto: massimo 3 ingressi al giorno.', 'stato': 'limite_turni'}, 400)
    ...
    if p.ora_entrata and not p.ora_uscita:
        ...
        return ({
            'ok': False,
            'error': 'Nessun turno aperto da chiudere. Registra prima un check-in.',
            'stato': 'nessun_turno_aperto',
        }, 400)
...
def candidato_timbratura_geo(request):
    ...
    payload, st = timbratura_geo_processo(request, dip, azione, lat, lon, acc)
