...
urlpatterns = [
    ...
    path('partitario-netti/', include('partitario_netti.urls')),
...
if settings.DEBUG:
    if getattr(settings, "GESPER_SANDBOX_ENABLED", False):
        from sandbox_dimostrativo.views_media import serve_debug_media_multiroot

        urlpatterns += [
            re_path(r"^media/(?P<path>.*)$", serve_debug_media_multiroot, name="gesper_debug_media"),
        ]
    else:
