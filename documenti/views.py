...
def verifica_buste_paga(request):
    ...
    Non esposto ai consulenti (strumenti di conciliazione / motore riservati ad admin e HR).
    """
    if not _is_admin_or_hr(request.user):
        return HttpResponseForbidden(
            "Accesso riservato a amministratori di piattaforma o HR."
        )
    return render(
        request,
...
def _allowed_documento_media_subdirs_set() -> frozenset[str]:
    ...
    def _scan_tree(media_root: Path) -> None:
...
def _resolve_media_path_relative_to_root(rel: str) -> Path | None:
    """Primo path assoluto sicuro dove il file esiste (sandbox poi operativo in demo)."""
    from sandbox_dimostrativo.media_paths import gesper_media_roots_for_read
    ...
    if not parts:
        return None
    for media in gesper_media_roots_for_read():
        try:
...
def _lista_pdf_browse_sottocartella_merged(browse_subdir: str, max_files: int) -> tuple[list[dict[str, object]], bool]:
    """Elenco PDF unendo sandbox + operativo (stesso ``rel``: vince la sandbox)."""
    from sandbox_dimostrativo.media_paths import gesper_media_roots_for_read

    merged: dict[str, dict[str, object]] = {}
    truncated = False
    for media_root in gesper_media_roots_for_read():
        part, trunc = _lista_pdf_in_sottocartella(media_root, browse_subdir, max_files)
        if trunc:
            truncated = True
        for e in part:
...
def admin_archivio_documenti_elimina_file(request):
    ...
    from sandbox_dimostrativo.media_paths import media_path_is_under_sandbox_writable

    if getattr(request, "gesper_sandbox", False) and not media_path_is_under_sandbox_writable(path):
        messages.error(
            request,
            "In modalità dimostrativa non è consentito eliminare file dell'archivio operativo.",
        )
        return redirect("admin_archivio_documenti_storage")
...
def admin_archivio_documenti_storage(request):
    ...
    from documenti.upload_paths import all_documento_storage_subdirs
    from sandbox_dimostrativo.media_paths import gesper_media_root_write

    media_root = gesper_media_root_write()
    ...
    if request.method == "POST":
        ...
        if action == "create_folder":
            ...
            elif getattr(request, "gesper_sandbox", False) and not media_path_is_under_sandbox_writable(src):
                messages.error(
                    request,
                    "In modalità dimostrativa non è consentito spostare file dell'archivio operativo.",
                )
    ...
    if browse_subdir and browse_subdir in _allowed_documento_media_subdirs_set():
        browse_files, browse_truncated = _lista_pdf_browse_sottocartella_merged(
            browse_subdir, ARCHIVIO_DOC_BROWSE_MAX_PDF
        )
