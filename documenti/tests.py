...
class ConciliazioneChecksTests(TestCase):
	def test_formula_ko_bloccanti_ignora_f_standard(self):
		from types import SimpleNamespace
		...
		ch_f6 = SimpleNamespace(ok=False, campo="F6 · Imponibile IRPEF mensile")
		...
		self.assertEqual(formula_ko_bloccanti_da_checks([ch_f5]), 0)
		self.assertEqual(formula_ko_bloccanti_da_checks([ch_f3, ch_f5]), 0)
		self.assertEqual(formula_ko_bloccanti_da_checks([ch_f6]), 1)
