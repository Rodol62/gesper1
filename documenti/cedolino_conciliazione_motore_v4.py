...
def conciliazione_oggi_vs_cedolino_motore_v4(
    ...
    *,
    ...
    # Non usare ``or``: ``Decimal('0.00')`` è falsy e con imponibile zero si perdeva la lettura PDF.
    _im_soc = _tot(report, "Imponibile Contr. Soc.")
    _im_alt = _tot(report, "Imponibile contributivo")
    impon_let = _im_soc if _im_soc is not None else _im_alt
        ...
        righe.append(
            {
                ...
                "nota": (
                    ...
                    "Per il badge OK/Δ contano le righe «elaborato vs PDF» sopra (lordo, netto, imponibile, contributi); "
                    "le formule F1–F9 in elenco sono diagnostiche (F5/F8 possono segnalare differenze tra modello semplificato "
                    "e totali TeamSystem pur con PDF coerente)."
                ),
            }
        )
