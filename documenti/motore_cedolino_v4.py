...
COORDINATE STABILI (top px, tolleranza ±4):
  ...
  ~648–688   Ferie / permessi / ROL (etichette + riga valori; Y variabile)
  ~664       Riga IRPEF erario / addizionali / conguaglio (±) / netto (varianti TS)
  ~696–712   Etichette + riga dati statistici INPS/INAIL/TFR (POS.SETT. lettera «O» = zero)
  ~727–736   Detrazioni spettanti (valore x≈55–70)
  ~752–760   Progressivi annui (7 valori tipici; 8° colonna oneri deduc. se presente)
─────────────────────────────────────────────────────────────────────────────

Mappa sezioni «Mod. Cedolino TS» (layout Studio Cipriano / TeamSystem) → motore:
  • Intestazione            → ``Cedolino`` (parse_intestazione + matricole INPS/INAIL)
  • Voci retributive        → retribuzione base (parse_retr_base_posizionale) + ``voci``
  • Previdenziale / INPS    → riga totali A–D (``imponibile_contrib``, ``tot_contrib_soc``) +
                              dati statistici (``inps.*``)
  • Fiscale (IRPEF mese)    → righe B–E (``imp_irpef_mese`` … ``addiz_*``) + detrazioni spettanti
  • Conguaglio / cessazione → ``conguaglio_irpef`` (riga addizionali), ``tipo_cedolino``,
                              ``data_cessazione``
  • TFR / INAIL             → ``inps.tfr_mese``, ``inps.*inail*`` (stessa riga dati statistici)
  • Progressivi annuali     → ``prog.*`` (banda sotto le etichette PROGRES./ANNUI)
...
Y_RIGA_FERIE = (640, 692,  10)  # Ferie/ROL (varianti TS con riga valori sfasata)
Y_RIGA_FEST  = (682, 695,  8)   # Festività extra
# Dati statistici INPS/INAIL/TFR: su alcune stampe TS la riga valori scorre di ~10 px
Y_RIGA_INPS  = (698, 728,  12)  # Dati INPS/INAIL/TFR (etichette + riga numeri)
Y_RIGA_DETR  = (726, 748,  10)  # Detrazioni spettanti (singolo valore x≈55–70)
Y_RIGA_PROG  = (748, 778,  12)  # Progressivi annui (7–8 valori; Y variabile)
...
# Colonne centrali riga E: conguaglio IRPEF (±) tra addizionali e arr/netto
X_CONGUAGLIO_IRPEF = (210, 285)
...
def _token_valore_cedolino_ts(text: str) -> float | None:
    """
    Numero da cella cedolino: importo italiano ``1.234,56`` / ``-19,98`` oppure intero ``25`` (GG).
    """
    ...
    if "," not in raw:
        return None
    if not re.match(r"^-?[\d.']+,\d+$", raw.replace("'", "")):
        return None
    return nf(raw)


def _pos_sett_da_token(text: str) -> int | None:
    """
    Posizione settimanale INPS: di norma 0–7 o simbolo «O» (zero) sul PDF.
    Non confondere con «25» (GG INAIL) o altri interi a due cifre.
    """
...
def parse_conguaglio_irpef_posizionale(words, c: Cedolino) -> None:
    """
    Conguaglio IRPEF (±) sulla stessa riga di IRPEF erario / addizionali (layout TS riga E).
    Preferisce un importo negativo in banda ``X_CONGUAGLIO_IRPEF`` se presente.
    """
    row_map: Dict[int, List[Tuple[float, float, str]]] = {}
    for w in words:
        if not (620 <= w["top"] <= 678):
            continue
        s = w["text"].strip()
        v = _token_valore_cedolino_ts(s)
        if v is None:
            continue
        y = round(w["top"] / 2) * 2
        row_map.setdefault(y, []).append((w["x0"], v, s))

    def col(items: List[Tuple[float, float, str]], x0: float, x1: float,
             vmin: float | None = None, vmax: float | None = None) -> float | None:
        for x, v, _t in items:
            if not (x0 <= x <= x1):
                continue
    ...
    for _y, items in sorted(row_map.items()):
        items = sorted(items, key=lambda t: t[0])
        ir = col(items, 43, 75, 10, 900)
        ar2 = col(items, 98, 130, 0, 400)
        if ir is None or ar2 is None:
            continue
        band = [(x, v) for x, v, _s in items if X_CONGUAGLIO_IRPEF[0] <= x <= X_CONGUAGLIO_IRPEF[1]]
        if not band:
            continue
        neg = [v for _x, v in band if v < -0.009]
        if neg:
            c.conguaglio_irpef = ar(neg[0])
            return
        pos = [v for _x, v in band if abs(v) >= 0.009]
        if pos:
            c.conguaglio_irpef = ar(pos[-1])
            return


def parse_dati_statistici_inps_inail_tfr(words, c: Cedolino) -> None:
    """
    Riga «Q/ POS. SETT. … TFR MESE» (valori sotto le etichette INPS/INAIL).
    Gestisce 6–8 token numerici (``25`` senza decimali per GG/ore INAIL).
    """
    row_map: Dict[int, List] = {}
    for w in words:
        if not in_y(w["top"], Y_RIGA_INPS):
            continue
        y = round(w["top"] / 2) * 2
        row_map.setdefault(y, []).append(w)

    best_ws: List | None = None
    best_n = 0
    for _y, ws in row_map.items():
        xs = sorted(ws, key=lambda w: w["x0"])
        ntok = 0
        for w in xs:
            t = w["text"].strip()
            if _pos_sett_da_token(t) is not None and "," not in t:
                ntok += 1
                continue
            if _token_valore_cedolino_ts(t) is not None:
                ntok += 1
        if ntok >= 5 and ntok > best_n:
            best_n = ntok
            best_ws = xs
    ...
    pos_sett: int | None = None
    nums: List[Tuple[float, float]] = []
    for w in best_ws:
        t = w["text"].strip()
        ps = _pos_sett_da_token(t)
        if ps is not None and "," not in t:
            pos_sett = ps
            continue
        v = _token_valore_cedolino_ts(t)
        if v is None:
            continue
        nums.append((w["x0"], v))
    nums.sort(key=lambda t: t[0])
    vals = [v for _x, v in nums]

    if pos_sett is not None:
        c.inps.pos_sett = pos_sett

    if len(vals) >= 7:
        c.inps.ore_inps = ar(vals[0])
        c.inps.gg_inps = ar(vals[1])
        c.inps.gg_minim = ar(vals[2])
        c.inps.ore_inail = ar(vals[3])
        c.inps.gg_inail = ar(vals[4])
        c.inps.imponibile_inail = ar(vals[5])
        c.inps.tfr_mese = ar(vals[6])
    elif len(vals) == 6:
        c.inps.ore_inps = ar(vals[0])
        c.inps.gg_inps = ar(vals[1])
        c.inps.gg_minim = ar(vals[2])
        c.inps.ore_inail = 0.0
        c.inps.gg_inail = ar(vals[3])
        c.inps.imponibile_inail = ar(vals[4])
        c.inps.tfr_mese = ar(vals[5])


def parse_progressivi_annui_posizionale(words, c: Cedolino) -> None:
    """
    Sette valori cumulati sotto «PROGRES. / ANNUI» (layout TS compatto).

    Con 7 colonne (senza «oneri deducibili» separato) si valorizza ``imp_irpef``…``irpef_pagata``
    e si lascia ``oneri_deduc`` a 0; l'ordine è quello crescente di ``x0`` sul PDF.
    """
    row_map: Dict[int, List] = {}
    for w in words:
        if not in_y(w["top"], Y_RIGA_PROG):
            continue
        s = w["text"].strip()
        if _token_valore_cedolino_ts(s) is None:
            continue
        y = round(w["top"] / 2) * 2
        row_map.setdefault(y, []).append(w)
    ...
    for w in sorted(best, key=lambda w: w["x0"]):
        v = _token_valore_cedolino_ts(w["text"].strip())
        if v is not None:
            vals.append(ar(v))

    if len(vals) >= 8:
        c.prog.imp_inail = vals[0]
        c.prog.imp_contrib_soc = vals[1]
        c.prog.contrib_soc = vals[2]
        c.prog.oneri_deduc = vals[3]
        c.prog.imp_irpef = vals[4]
        c.prog.irpef_lorda = vals[5]
        c.prog.tot_detr = vals[6]
        c.prog.irpef_pagata = vals[7]
    elif len(vals) == 7:
        c.prog.imp_inail = vals[0]
        c.prog.imp_contrib_soc = vals[1]
        c.prog.contrib_soc = vals[2]
        c.prog.oneri_deduc = 0.0
        c.prog.imp_irpef = vals[3]
        c.prog.irpef_lorda = vals[4]
        c.prog.tot_detr = vals[5]
        c.prog.irpef_pagata = vals[6]


def parse_detrazioni_spettanti_posizionale(words, c: Cedolino) -> None:
    """Singolo importo sotto «DETRAZIONI SPETTANTI» (totale spettante del mese)."""
    cand: List[Tuple[float, float]] = []
    for w in words:
        if not in_y(w["top"], Y_RIGA_DETR):
            continue
        v = _token_valore_cedolino_ts(w["text"].strip())
    ...
    tot = ar(cand[0][1])
    c.detr.lavoro_dip = tot
    c.detr.totale = tot


def parse_ferie_permessi_posizionale(words, c: Cedolino) -> None:
    """
    Ferie / permessi / ROL da righe numeriche nella banda bassa (evita la riga IRPEF/addizionali).
    """
    row_map: Dict[int, List[Tuple[float, float, str]]] = {}
    for w in words:
        if not in_y(w["top"], Y_RIGA_FERIE):
            continue
        s = w["text"].strip()
        v = _token_valore_cedolino_ts(s)
        if v is None:
    ...
    def col(items: List[Tuple[float, float, str]], x0: float, x1: float) -> float | None:
        for x, v, _s in items:
            if x0 <= x <= x1:
                return v
        return None
    ...
    for _y, items in sorted(row_map.items()):
        items = sorted(items, key=lambda t: t[0])
        ir = col(items, 43, 75)
        if ir is not None and col(items, 98, 130) is not None:
            continue
        if len(items) >= 8:
            c.ferie_perm.ferie_ap = ar(col(items, 35, 52) or 0.0)
            c.ferie_perm.ferie_mat = ar(col(items, 58, 78) or 0.0)
            c.ferie_perm.ferie_god = ar(col(items, 92, 112) or 0.0)
            c.ferie_perm.ferie_res = ar(col(items, 128, 150) or 0.0)
            c.ferie_perm.perm_ap = ar(col(items, 162, 182) or 0.0)
            c.ferie_perm.perm_mat = ar(col(items, 196, 218) or 0.0)
            c.ferie_perm.perm_god = ar(col(items, 222, 252) or 0.0)
            c.ferie_perm.perm_res = ar(col(items, 258, 282) or 0.0)
            c.ferie_perm.rol_ap = ar(col(items, 296, 318) or 0.0)
            c.ferie_perm.rol_mat = ar(col(items, 324, 348) or 0.0)
            c.ferie_perm.rol_god = ar(col(items, 352, 378) or 0.0)
            c.ferie_perm.rol_res = ar(col(items, 384, 408) or 0.0)
            return
        if len(items) == 2:
            x0, v0, _s0 = items[0]
            _x1, v1, _s1 = items[1]
            if 95 <= x0 <= 125 and 125 <= items[1][0] <= 155:
                c.ferie_perm.rol_god = ar(v0)
                c.ferie_perm.rol_res = ar(v1)
                return
...
def parse_from_words_testo(words, testo: str, file_pdf: str) -> Cedolino:
    ...
    parse_conguaglio_irpef_posizionale(words, c)
    parse_dati_statistici_inps_inail_tfr(words, c)
    parse_progressivi_annui_posizionale(words, c)
    parse_detrazioni_spettanti_posizionale(words, c)
    parse_ferie_permessi_posizionale(words, c)
...
def cedolino_sezioni_ts(c: Cedolino, calc: dict[str, Any]) -> dict[str, Any]:
    """
    Struttura ad albero allineata alle macro-aree del cedolino TeamSystem
    (intestazione, voci, previdenziale, fiscale, conguaglio/cessazione, TFR, INAIL, progressivi).
    Utile per JSON, template o conciliazione senza dipendere dall'ordine di ``stampa()``.
    """
    fp = c.ferie_perm
    inps = c.inps
    prog = c.prog
    ...
    return {
        "intestazione": {
            "file_pdf": c.file_pdf,
            "tipo_cedolino": c.tipo_cedolino,
            "mese_anno": c.mese_anno,
            "foglio_n": c.foglio_n,
            "cognome_nome": c.cognome_nome,
            "codice_fiscale": c.codice_fiscale,
            "data_nascita": c.data_nascita,
            "comune_residenza": c.comune_residenza,
            "data_assunzione": c.data_assunzione,
            "data_cessazione": c.data_cessazione,
            "matricola": c.matricola,
            "matr_inps_az": c.matr_inps_az,
            "pos_inail": c.pos_inail,
            "codice_dip": c.codice_dip,
            "qualifica": c.qualifica,
            "livello": c.livello,
            "gg_contratto": c.gg_contratto,
            "ore_contratto": c.ore_contratto,
        },
        "descrizione_voci_retributive": {
            "paga_base": c.paga_base,
            "contingenza": c.contingenza,
            "el_dis_san": c.el_dis_san,
            "el_dis_bil": c.el_dis_bil,
            "retr_oraria_att": c.retr_oraria_att,
            "retr_giornaliera": c.retr_giornaliera,
            "retrib_di_fatto": c.retrib_di_fatto,
            "voci": [
                {
                    "codice": v.codice,
        ...
        "previdenziale": {
            "imponibile_contrib": c.imponibile_contrib,
            "contrib1": c.contrib1,
            "tot_contrib_soc": c.tot_contrib_soc,
            "pos_sett_inps": inps.pos_sett,
            "ore_inps": inps.ore_inps,
            "gg_inps": inps.gg_inps,
            "gg_minimi": inps.gg_minim,
        },
        "fiscale": {
            "totale_lordo": c.totale_lordo,
            "imp_irpef_mese": c.imp_irpef_mese,
            "irpef_lorda_mese": c.irpef_lorda_mese,
            "tot_detr_mese": c.tot_detr_mese,
            "tot_trat_irpef": c.tot_trat_irpef,
            "arr_prec": c.arr_prec,
            "tot_trattenute": c.tot_trattenute,
            "netto_busta": c.netto_busta,
            "irpef_erario": c.irpef_erario,
            "addiz_regionale": c.addiz_regionale,
            "addiz_comunale": c.addiz_comunale,
            "arr_attuale": c.arr_attuale,
            "detrazioni_spettanti": {
                "lavoro_dipendente": detr.lavoro_dip,
                "coniuge": detr.coniuge,
                "figli": detr.figli,
                "altri_carichi": detr.altri_carichi,
                "totale": detr.totale,
            },
            "ferie_permessi_rol": {
                "ferie": {
        ...
        "conguaglio_fine_anno_cessazione": {
            "conguaglio_irpef": c.conguaglio_irpef,
            "tipo_cedolino": c.tipo_cedolino,
            "data_cessazione": c.data_cessazione,
        },
        "tfr": {
            "tfr_mese": inps.tfr_mese,
            "tfr_minimo_teorico": calc.get("tfr_minimo_teorico"),
            "tfr_delta_su_minimo": calc.get("tfr_delta_su_minimo"),
        },
        "inail": {
            "ore_inail": inps.ore_inail,
            "gg_inail": inps.gg_inail,
            "imponibile_inail": inps.imponibile_inail,
        },
        "progressivi_annui": {
            "imponibile_inail": prog.imp_inail,
            "imponibile_contributivo_sociale": prog.imp_contrib_soc,
            "contributi_sociali": prog.contrib_soc,
            "oneri_deducibili": prog.oneri_deduc,
            "imponibile_irpef": prog.imp_irpef,
            "irpef_lorda": prog.irpef_lorda,
            "totale_detrazioni": prog.tot_detr,
            "irpef_pagata": prog.irpef_pagata,
        },
        "motore": {
            "retr_oraria_calc": calc.get("retr_oraria"),
            "totale_lordo_calc": calc.get("totale_lordo"),
            "imponibile_contrib_voci": calc.get("imponibile_contrib_voci"),
            "aliquota_inps": calc.get("aliquota_inps"),
        },
    }
...
def stampa(c: Cedolino, calc: dict, checks: list):
    ...
    # 1. Intestazione (layout cedolino TS)
    sep("1 · INTESTAZIONE")
    for lab, val in [
    ...
    # 2. Descrizione voci retributive
    sep("2 · DESCRIZIONE VOCI RETRIBUTIVE — componenti base")
    ...
    sep("   Verifica voci [F2: ore × base = importo]")
    ...
    # 3. Previdenziale (totali mensili + INPS)
    sep("3 · PREVIDENZIALE — contributi e dati INPS")
    r("Totale Lordo [PDF]", fmt(c.totale_lordo))
    r("Imponibile contributivo [PDF]", fmt(c.imponibile_contrib))
    r("Contributi sociali (tot.) [PDF]", fmt(c.tot_contrib_soc))
    aliq_info = f"{calc.get('aliquota_nome','?')} ({calc.get('aliquota_inps',0)*100:.2f}%)"
    r("Aliquota IVS dip. (motore F5)", aliq_info)
    inps = c.inps
    ...
    # 4. Fiscale (IRPEF mensile, trattenute, detrazioni, ferie/ROL)
    sep("4 · FISCALE — IRPEF, trattenute, detrazioni, ferie/ROL")
    r("Imponibile IRPEF mese [PDF]", fmt(c.imp_irpef_mese))
    r("IRPEF lorda mese [PDF]", fmt(c.irpef_lorda_mese))
    r("Tot. detrazioni IRPEF mese [PDF]", fmt(c.tot_detr_mese))
    r("Tot. trattenute IRPEF [PDF]", fmt(c.tot_trat_irpef))
    r("Arr. prec.", fmt(c.arr_prec))
    r("Tot. trattenute [PDF]", fmt(c.tot_trattenute))
    r("Netto busta [PDF]", fmt(c.netto_busta))
    ...
    if c.tipo_cedolino == "CESSAZIONE RAPPORTO" and "aliquota_tass_sep" in calc:
        r(f"Aliq. tass. separata ({fmt(calc['aliquota_tass_sep']*100,2)}%)",
          "IRPEF pag. ann. / Imp. ann.")
        r("IRPEF su preavviso (stima)", fmt(calc.get("irpef_preavviso", 0)))
    fp = c.ferie_perm
    print()
    ...
    print()
    r("Detraz. lavoro dipendente [PDF]", fmt(c.detr.lavoro_dip))
    r("Detraz. coniuge", fmt(c.detr.coniuge))
    r("Detraz. figli", fmt(c.detr.figli))
    r("Detraz. totale [PDF]", fmt(c.detr.totale))

    # 5. Conguaglio / fine anno / cessazione
    sep("5 · CONGUAGLIO IRPEF / FINE ANNO / CESSAZIONE RAPPORTO")
    r("Conguaglio IRPEF (±) [PDF]", fmt(c.conguaglio_irpef))
    r("Tipo cedolino", c.tipo_cedolino)
    r("Data cessazione", c.data_cessazione or "—")

    # 6. INAIL
    sep("6 · INAIL — dati mese")
    ...
    # 7. TFR
    sep("7 · TFR")
    r("TFR mese [PDF]", fmt(inps.tfr_mese))
    r("TFR minimo teorico (Imp.contrib./13,5)", fmt(calc.get("tfr_minimo_teorico", 0)))
    if "tfr_delta_su_minimo" in calc:
        delta_tfr = calc["tfr_delta_su_minimo"]
        perc_tfr = calc.get("tfr_perc_su_minimo", 0)
        r("Delta su minimo (quote diff. + rivalutazione)", f"{fmt(delta_tfr)} € (+{perc_tfr:.1f}%)")
        r("Nota", "TFR: pro-quota 13a/14a e rivalutazione non ricostruibili senza storico montante")

    # 8. Progressivi annuali
    sep("8 · PROGRESSIVI ANNUALI")
    p = c.prog
    r("Imponibile INAIL (prog.)", fmt(p.imp_inail))
    r("Imponibile contrib. sociale (prog.)", fmt(p.imp_contrib_soc))
    r("Contributi sociali (prog.)", fmt(p.contrib_soc))
    r("Oneri deducibili (prog.)", fmt(p.oneri_deduc))
    r("Imponibile IRPEF (prog.)", fmt(p.imp_irpef))
    r("IRPEF lorda (prog.)", fmt(p.irpef_lorda))
    r("IRPEF lorda [F7 scaglioni]", fmt(calc.get("irpef_lorda_annua", 0)))
    r("Tot. detrazioni (prog.)", fmt(p.tot_detr))
    r("IRPEF pagata (prog.)", fmt(p.irpef_pagata))

    # 9. Ricalcolo motore (tutte le formule F1–F9)
    sep("9 · RICALCOLO MOTORE — confronti lettura PDF")
    ct = [ch for ch in checks if "F2" not in ch.campo]
    ...
    # 10. Sigle
    if c.sigle:
        sep("10 · LEGENDA SIGLE")
        for sig, desc in c.sigle.items():
            r(sig, desc, 10)

    # 11. Riepilogo validazione
    sep("11 · RIEPILOGO VALIDAZIONE")
    ...
    # 12. Tavola formule
    sep("12 · TAVOLA FORMULE DI CALCOLO")
    formule = [
